...
export default function LabWizard({
  ...
  const downloadEvidence = () => {
    const content = [
      ...
      '',
      ...WIZARD_CHECKLIST.filter((section) =>
  ...
  const stageProps = {
    ...
    canContinue,
    checkpoints:
      currentSection && CORE_STEPS.includes(currentStep) ? (
        <CheckpointPanel
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Local demo, not a production system. Progress stays in this browser.
          </p>
        </div>
      ...
      <div className="mx-auto mt-6 max-w-4xl">
        ...
        {CORE_STEPS.includes(currentStep) && (
          <a
            href="#checkpoint-title"
        ...
        {currentStep === 'whatsnext' && coreComplete && (
          <WhatsNextStep {...stageProps} onReset={onReset} />
        )}
        ...
        <section className="mt-6 rounded-xl border border-slate-300 bg-white p-5 dark:border-slate-600 dark:bg-slate-800">
          ...
          >
            Optional: record expected vs. actual results and build output. Never
            paste secrets or personal data. Clearing browser data removes these
            notes; use Download evidence to keep a copy.
          </p>
          <textarea
            id="lab-notes"
            name="lab-notes"
