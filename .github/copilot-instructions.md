...
> 3. You watch and verify the results (be the agent's eyes!)
> 4. If there are ANY errors, copy the full error message AND take a screenshot, then paste both into Copilot Chat
> 5. Repeat until the error is resolved
> 6. Move to the next step
>
> **Error Handling Rule:** Never struggle with errors alone. Copy the entire error message and paste it directly into GitHub Copilot Chat in Beastmode (just the error, no prompt needed). The agent will automatically diagnose, explain, and fix it. Repeat until resolved.
>
> **Instructions for AI Agent:** When a user pastes an error message, automatically: (1) Explain what caused the error in simple terms, (2) Show exactly how to fix it, (3) Run the fix if possible, (4) Verify the error is resolved.
...
- **Styling:** Tailwind CSS (required for modern UI)
- **Power Apps Integration:** Power Platform SDK for authentication, connectors, and managed deployment
- **Data:** Dataverse (Microsoft's low-code data platform) via Power Platform connectors
